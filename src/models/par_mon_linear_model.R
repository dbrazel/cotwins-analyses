# Fit a multi-level linear growth model to parental monitoring using lme4
...
ml <-
  lmer(
    max_monitor_score ~ (test_age + I(test_age^2) + sex) + (test_age | family/user_id),
    data = par_mon
  )
...
write_rds(ml, "data/models/par_mon_linear_model.rds")
write_rds(par_mon_pred, "data/models/par_mon_linear_predictions.rds")
write_rds(parameters, "data/models/par_mon_linear_parameters.rds")
