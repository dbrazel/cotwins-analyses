# Fit a multi-level quadratic growth model to ecig uses per week using lme4 and
...
ml <-
  lmer(
    ecig_per_week ~ (test_age + I(test_age^2) + sex) + (test_age + I(test_age^2) | family/user_id),
    data = sub_use
  )
ml_test <-
  lmerTest::lmer(
...
write_rds(sub_use, "data/models/ecig_data.rds")
