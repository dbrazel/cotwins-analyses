# Fit a multi-level linear growth model to time at home at night using lme4
...
ml <-
  lmer(
    home_frac ~ (test_age + I(test_age^2) + sex) + (test_age | family/user_id),
    data = at_home
  )
...
write_rds(ml, "data/models/at_home_linear_model.rds")
write_rds(at_home_pred, "data/models/at_home_linear_predictions.rds")
write_rds(parameters, "data/models/at_home_linear_parameters.rds")
