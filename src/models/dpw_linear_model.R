# Fit a multi-level linear growth model to drinks per week (DPW) using lme4 and
...
ml <-
  lmer(
    drinks_per_week ~ (test_age + I(test_age^2) + sex) + (test_age | family/user_id),
    data = sub_use
  )
...
write_rds(ml, "data/models/dpw_linear_model.rds")
write_rds(sub_use_pred, "data/models/dpw_linear_predictions.rds")
write_rds(parameters, "data/models/dpw_linear_parameters.rds")
