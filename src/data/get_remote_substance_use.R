...
to_do_cols <- c(colnames(checkin)[c(4:11, 19, 21:25, 36:47, 50:55, 57:60)])
for (i in to_do_cols) {
...
checkin$cig_freq_days_per_week <- plyr::mapvalues(checkin$cig_freq_days_per_week, 
                                                  c('Seven days', 'Six days', 'Four days', 
                                                    'Five days', 'Two days', 'Three days', 'One day'), 
                                                  c(7, 6, 4, 5, 2, 3, 1), warn_missing = T) %>% as.numeric()

checkin$cig_quantity_per_day <- plyr::mapvalues(checkin$cig_quantity_per_day,
                                                c("2 cigarettes", "5 cigarettes per day", "5 cigarettes", 
                                                  "3 cigarettes", "3 cigarettes per day", "2 cigarettes per day", 
                                                  "4 cigarettes per day", "6-10 cigarettes", "1 cigarette per day", 
                                                  "Only part of a cigarette per day", "1 cigarette", "6-10 cigarettes per day", 
                                                  "More than 30 cigarettes", "11-15 cigarettes", "4 cigarettes"),
                                                c(2, 5, 5, 3, 3, 2, 4, 8, 1, 0.5, 1, 8, 30, 13, 4), warn_missing = T) %>%
                                                as.numeric()

checkin$cig_quantity_yesterday <- plyr::mapvalues(checkin$cig_quantity_yesterday, 
                                                  c("5 cigarettes", "1 cigarette", "2 cigarettes", "3 cigarettes", 
                                                    "0 cigarettes (I did not smoke at all yesterday)", "6-10 cigarettes", 
                                                    "Only part of a cigarette", "4 cigarettes", "11-15 cigarettes"), 
                                                  c(5, 1, 2, 3, 0, 8, 0.5, 4, 13), warn_missing = T) %>% as.numeric()

checkin$ecig_freq_days_per_week <- plyr::mapvalues(checkin$ecig_freq_days_per_week, 
                                                   c('Seven days', 'Six days', 'Four days', 
                                                     'Five days', 'Two days', 'Three days', 'One day'), 
                                                  c(7, 6, 4, 5, 2, 3, 1), warn_missing = T) %>% as.numeric()

checkin$ecig_freq_times_per_day <- plyr::mapvalues(checkin$ecig_freq_times_per_day, 
                                                   c("Once per day", "2-3 times per day", "4-5 times per day", 
                                                     "10 or more times per day", "6-9 times per day"), 
                                                   c(1, 2.5, 4.5, 10, 7.5), warn_missing = T) %>% as.numeric()

checkin$ecig_quantity_puffs <- plyr::mapvalues(checkin$ecig_quantity_puffs, 
                                               c("2-3 puffs", "One puff", "4-5 puffs", "10 or more puffs", "6-9 puffs"), 
                                                   c(2.5, 1, 4.5, 10, 7.5), warn_missing = T) %>% as.numeric()
