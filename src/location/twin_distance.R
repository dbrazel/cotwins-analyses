...
id_mapping <- bind_rows(tibble(SVID = id_mapping$T1, Michigan_ID = id_mapping$T1_alternate_id), 
    tibble(SVID = id_mapping$T2, Michigan_ID = id_mapping$T2_alternate_id))
...
for (twin_pair in seq(to = nrow(twin_info), by = 2)) {
    t1_locs <- filter(std_locs, Michigan_ID == twin_info$Michigan_ID[twin_pair])
    t2_locs <- filter(std_locs, Michigan_ID == twin_info$Michigan_ID[(twin_pair + 1)])
    ...
    tp_dists <- bind_rows(tp_dists, tibble(tp_id = t_locs$Michigan_ID.x, 
        zygosity = rep(twin_info$bestzygos[twin_pair], nrow(t_locs)), DateTime = t_locs$DateTime,
        distance = t_dist))

    tp_mean_dists[nrow(tp_mean_dists) + 1, ] <- list(twin_info$Michigan_ID[twin_pair], 
