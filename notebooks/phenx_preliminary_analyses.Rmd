...
Out of `r nrow(phenx)` twins, `r joined %>% filter(Sex1 == 'Male') %>% nrow()` male and `r joined %>% filter(Sex1 == 'Female') %>% nrow()` female, `r nrow(ever_drinkers)` admit to having at least one drink of alcohol. `r nrow(drinkers)` admit to having had more than five drinks. The mean age of drinking initiation was `r comma(mean(ever_drinkers$PHXQ0003))`, with this distribution, which shows some obvious outliers:
...
If we fit a linear model, we can see that age in years is not significantly correlated with days of drinking if we include drinkers who did not drink in the past month as zeroes.
