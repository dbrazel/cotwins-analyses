...
```{r}
psych::r.test(n = nrow(dz_data), n2 = nrow(mz_data), r12 = cor(dz_data$T1_N, dz_data$T2_N), r34 = cor(mz_data$T1_N, mz_data$T2_N))
```

Let's plot what this looks like.
```{r}
id_mapping$bestzygos[id_mapping$bestzygos == "OS"] <- "DZ"
ggplot(id_mapping, aes(T1_N, T2_N, color = bestzygos)) + geom_point() + geom_smooth(method = "lm")
```
